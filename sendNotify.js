 ...
 * @Last Modified by: lazzman
 * @Last Modified time: 2022-01-13 10:00:00
 * sendNotify 推送通知功能
 * @param text 通知头
 * @param desp 通知体
...
console.log("加载sendNotify，当前版本: 20220113");
// =======================================go-cqhttp通知设置区域===========================================
...
环境变量名 QYWX_PERSONAL_NOTIFY依次填入pt_pin,qywx_uid
例如：pt_pin=jd_fFtIGsfGQgEM,qywx_uid=LazzMan
当配置了QYWX_PERSONAL_NOTIFY时，QYWX_AM中的touser将失效（其它参数依然生效，也就是说QYWX_AM环境变量不能少），企业微信消息推送接收人将使用QYWX_PERSONAL_NOTIFY配置的人员，实现个人消息推送。
 */
let QYWX_AM = '';
// 企业微信个人推送
let QYWX_PERSONAL_NOTIFY = '';
...
async function sendNotify(text, desp, params = {}, author = '\n\n本通知 By LazzMan Modify', strsummary = "", pt_pin = '') {
    console.log(`开始发送通知...`);
    ...
    await Promise.all([
        ...
        qywxamNotify(text, desp, strsummary, pt_pin), //企业微信应用消息推送
...
function ChangeUserId(desp, pt_pin) {
    const QYWX_AM_AY = QYWX_AM.split(',');
    if (pt_pin) {
        // 向个人推送
        return GetQywxUid(pt_pin);
    } else {
        // 向QYWX_AM中配置的用户推送
        if (QYWX_AM_AY[2]) {
 ...
 * 获取企业微信uid
 * @param pt_pin
 * @return 在环境变量配置中找到个人推送 QYWX_PERSONAL_NOTIFY 中的qywx_uid参数值
 */
function GetQywxUid(pt_pin) {
    pt_pin = decodeURIComponent(pt_pin);
    pt_pin = pt_pin.substring(0, pt_pin.indexOf(";") > 0 ? pt_pin.indexOf(";") : pt_pin.length);
    QYWX_PERSONAL_NOTIFY = process.env.QYWX_PERSONAL_NOTIFY;
    const QYWX_PERSONAL_NOTIFY_AY = QYWX_PERSONAL_NOTIFY.split('&');
    for (let qpn of QYWX_PERSONAL_NOTIFY_AY) {
        if (qpn.includes(pt_pin)) {
            return qpn.split(';')[1].split('=')[1];
        }
    }
    return '';
}
 ...
 * @param text 标题
 * @param desp 描述
 * @param strsummary 详细描述
 * @param pt_pin 京东cookie，默认为空，则向QYWX_AM环境变量中配置的TOUSER推送消息，若不为空，则向个人推送消息，格式为 pt_pin=jd_fFtIGsfGQgEM;
 * @returns {Promise<unknown>}
 */
function qywxamNotify(text, desp, strsummary = "", pt_pin = "") {
    return new Promise((resolve) => {
        if (QYWX_AM) {
            const QYWX_AM_AY = QYWX_AM.split(',');
            ...
            $.post(options_accesstoken, (err, resp, data) => {
                ...
                let touser = ChangeUserId(desp, pt_pin);
                if (touser) {
                    options = {
                    ...
                    $.post(options, (err, resp, data) => {
                        try {
                            if (err) {
                                console.log('成员ID:' + ChangeUserId(desp, pt_pin) + '企业微信应用消息发送通知消息失败！！\n');
                                console.log(err);
                            } else {
                                data = JSON.parse(data);
                                if (data.errcode === 0) {
                                    console.log('成员ID:' + ChangeUserId(desp, pt_pin) + '企业微信应用消息发送通知消息成功🎉。\n');
                                } else {
                                    console.log(`${data.errmsg}\n`);
                                }
                            }
